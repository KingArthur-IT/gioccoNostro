<template>
  <div class="custom-input">
    <div v-if="isHeadInfoShown()" class="custom-input__upper-info" :class="{'w-100': label === ''}">
      ...
      <p class="custom-input__required" v-if="showRequiredInfo">{{ $t('required_fields') }}<sup>*</sup></p>
    ...
    <div v-if="isShowWarningLabel && isFocused" class="warning-label">
      <p class="warning-label__title">{{ $t('warning') }}</p>
      <p class="warning-label__text">{{warningText}}</p>
    </div>
    <EyeIcon v-if="isPassword" :isOpened="idPasswordVisible" @click="idPasswordVisible = !idPasswordVisible"
             class="password-eye"/>
  </div>
</template>
