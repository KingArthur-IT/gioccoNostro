...
export default {
    ...
    methods:{
        ...
        async orderEvent(ident){
            await axios.post('https://api.gioconostro.com/api/v1/game/order', 
            {
                ident: ident
            },
            {
            ...
            .then((response) => {
                if (response && response.data && response.data.status === 'success'){
                    this.orderGamePrice = response.data.data.order.game_price;
                    this.orderGameTotal = response.data.data.order.total_to_pay;
        ...
        paymentEvent(){
            ...
            this.isShowAlertModal = true;
            this.alertModalText = 'Here will be redirect to payment system page';
        }
    },
    watch:{
